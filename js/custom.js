...
function formatC(data) {
	if (data != '0' && data) {
		data = data.toString().replace(/[\D\s\._\-]+/g, "");
		data = data ? parseInt(data, 10) : 0;
		return (data === 0) ? "" : data.toLocaleString("en-US");
	}
	return 0;
}

function unformatC(data) {
	if (data != '0') {
		return parseInt(data.toString().replace(/[\D\s\._\-]+/g, ""));
	} else {
		return 0;
	}
}
...
$("body").on("keyup", ".currency", function () {
	if ($(this).val() != '0') {
		var input = $(this).val().toString().replace(/[\D\s\._\-]+/g, "");
...
function init_sidebar() {
	...
	$SIDEBAR_MENU.find('a').filter(function () {
		return this.href.toLowerCase() == CURRENT_URL.toLowerCase();
