...
class m_voucher_tagihan_virtual extends CI_Model
{
    public function get_voucher($t1, $t2)
    {
        ...
        $voucher_biaya_admin = $this->db
            ->select("
                ...
                view_coa.coa as coa_cara_pembayaran,
                SUM(isnull(nilai_biaya_admin_cara_pembayaran,0)) as tagihan
            ")
            ->from("t_pembayaran")
            ->join("unit_virtual", "unit_virtual.id = t_pembayaran.unit_id")
            ->join("customer", "customer.id = unit_virtual.customer_id")
            ->join("dbmaster.dbo.m_pt", "m_pt.pt_id = customer.pt_id")
            ->join("cara_pembayaran", "cara_pembayaran.id = t_pembayaran.cara_pembayaran_id")
            ->join("bank", "bank.id = cara_pembayaran.bank_id", "LEFT")
            ->join("gl_2018.dbo.view_coa", "view_coa.coa_id = cara_pembayaran.coa_mapping_id")
            ->where("unit_virtual.project_id", $project->id)
            ->where('CAST(t_pembayaran.tgl_bayar as date) >=', "$t1")
            ->where('CAST(t_pembayaran.tgl_bayar as date) <=', "$t2")
            ->where_in("t_pembayaran.is_void",[0, 2])
            ->group_by(" 
                m_pt.pt_id,
                m_pt.name,
                cara_pembayaran_id,
        ...
        foreach ($voucher as $v1) {
            foreach ($voucher_biaya_admin as $v2) {
                if($v1->pt_id == $v2->pt_id && 
                $v1->cara_pembayaran_id == $v2->cara_pembayaran_id)
                    $v1->tagihan += $v2->tagihan;
            }
        }
    ...
    public function get_detail($pt_id, $cara_pembayaran_id, $t1, $t2)
    {
        ...
        $data = $this->db
            ->select("
                v_voucher_virtual.t_pembayaran_id,
                v_voucher_virtual.unit_id,
                v_voucher_virtual.kawasan,
                v_voucher_virtual.blok,
                v_voucher_virtual.no_unit,
                v_voucher_virtual.pemilik,
                v_voucher_virtual.item,
                v_voucher_virtual.nilai_tagihan,
                v_voucher_virtual.nilai_ppn,
                v_voucher_virtual.nilai_denda,
                v_voucher_virtual.nilai_penalti,
                v_voucher_virtual.nilai_biaya_admin_cara_pembayaran,
                FORMAT (v_voucher_virtual.tgl_bayar,'dd-MM-yyyy') as tgl_bayar,  
            ...
            ->join(
                "service",
                "service.id = v_voucher_virtual.service_id"
            )
            ->join(
            ...
            ->join(
                "cara_pembayaran",
                "cara_pembayaran.id = '$cara_pembayaran_id'",
                "LEFT")
            ->join(
                "gl_2018.dbo.view_coa as coa_cara_pembayaran",
                "coa_cara_pembayaran.coa_id = cara_pembayaran.coa_mapping_id",
                "LEFT"
            )
            ->where("v_voucher_virtual.cara_pembayaran_id", $cara_pembayaran_id)
            ->where("v_voucher_virtual.pt_id", $pt_id)
            ->WHERE("CAST(tgl_bayar as date) BETWEEN '$t1' AND '$t2' ")
            // ->where('tgl_bayar >=', "$t1")
            // ->where('tgl_bayar <=', "$t2")
            ->where('v_voucher_virtual.project_id', $project->id)
        ...
        foreach ($data as $k => $v) {
            ...
            if ($v->nilai_biaya_admin_cara_pembayaran > 0) {
                ...
                if($k == 0 || $data[$k-1]->t_pembayaran_id != $v->t_pembayaran_id){
                    ...
                    $tmp['nilai_item'] = $v->nilai_biaya_admin_cara_pembayaran;
                    array_push($data_detil,$tmp);
                    $total = $total + $v->nilai_biaya_admin_cara_pembayaran;
                }
            }
    ...
    public function get_detail_gabungan($pt_id, $cara_pembayaran_id, $t1, $t2)
    {
        ...
        $data = $this->db->select("
                            ...
                            sum(v_voucher_virtual.nilai_tagihan) as nilai_tagihan,
                            sum(v_voucher_virtual.nilai_ppn) as nilai_ppn,
                            sum(v_voucher_virtual.nilai_denda) as nilai_denda,
                            sum(v_voucher_virtual.nilai_penalti) as nilai_penalti,
                            coa_tagihan.coa as coa_tagihan,
                            coa_ppn.coa as coa_ppn,
                            coa_denda.coa as coa_denda
                        ")
