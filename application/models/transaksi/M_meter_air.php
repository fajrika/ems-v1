...
class m_meter_air extends CI_Model
{
    ...
    public function harga_air($data1, $data2, $data3)
    {
        ...
        if ($rumus == 1) {
            ...
            if   (isset($result[$i])) {
                do {
                    if ($data3 > ($result[$i]['range_akhir'] - ($i != 0 ? $result[$i - 1]['range_akhir'] : 0))) { 
        ...
        if (true) {
            ...
            if ($meter_awal == 0) {
                $meter_awal = $this->db->select("meter_akhir")
                    ...
                    ->get()->row();
                $meter_awal = $meter_awal->meter_akhir ?? 0;
            }
            $dataMeterAir = [
                'unit_id'       => $unit_id,
                'periode'       => $periode,
            ...
            $resultMeterAir = $this->db->query("SELECT id FROM t_pencatatan_meter_air WHERE periode = '$dataMeterAir[periode]' AND unit_id = $dataMeterAir[unit_id]")->row();
            $this->db->trans_start();
            ...
            $resultTagihan = $this->db
                ...
                ->join('range_air', 'range_air.id = sub_golongan.range_id')
            ...
            $resultTagihanAir = $this->db->query("SELECT id FROM t_tagihan_air WHERE periode = '$dataTagihanAir[periode]' AND unit_id = $dataTagihanAir[unit_id]")->row();
    ...
    public function ajax_save_meter_api_1($meter, $periode, $unit_id, $url)
    {
        ...
        if ($resultMeterAir) { //jika t_meter_air sudah ada, maka di update
            ...
            $this->db->insert('t_pencatatan_meter_air', $dataMeterAir);
            $dataMeterAir_id = $this->db->insert_id();
            $t_pencatatan_meter_air_detail = (object)[];

            $t_pencatatan_meter_air_detail->t_pencatatan_meter_air_id = $this->db->insert_id();
            $t_pencatatan_meter_air_detail->create_at = date("Y-m-d H:i:s.000");
            foreach ($url as $ind => $el) {
                $t_pencatatan_meter_air_detail->foto_url = $el;
