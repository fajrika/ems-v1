...
class m_tagihan extends CI_Model
{
    ...
    public function pl2($project_id, $periode)
    {
        ...
        foreach ($data as $dv) {
            ...
            if (!$tagihan_sudah_ada->num_rows()) {
                ...
                $data_lingkungan_detail->nilai_bangunan        = $biaya_bangunan ?? 0;
                $data_lingkungan_detail->nilai_kavling         = $biaya_kavling ?? 0;
                $data_lingkungan_detail->nilai_administrasi    = $dv->administrasi ?? 0;
                $data_lingkungan_detail->nilai_keamanan        = $dv->keamanan ?? 0;
                $data_lingkungan_detail->nilai_kebersihan      = $dv->kebersihan ?? 0;
                ...
                if (
                    ($data_lingkungan_detail->nilai_bangunan
                        + $data_lingkungan_detail->nilai_kavling
                        + $data_lingkungan_detail->nilai_administrasi
                        + $data_lingkungan_detail->nilai_keamanan
                        + $data_lingkungan_detail->nilai_kebersihan)
                    == 0
                )
                    $data_lingkungan->status_tagihan    = 1;
    ...
    public function ipl_unit($unit_id, $periode)
    {
        $unit = $this->db->select('*')->from('unit')->where('id', $unit_id)->get()->row();
        $unit_lingkungan = $this->db->select('*')->from('unit_lingkungan')->where('unit_id', $unit_id)->get()->row();
        $sub_golongan = $this->db->select('*')->from('sub_golongan')->where('id', $unit_lingkungan->sub_gol_id)->get()->row();
        $jenis_golongan = $this->db->select('*')->from('jenis_golongan')->where('id', $sub_golongan->jenis_golongan_id)->get()->row();
        $jenis_golongan = $this->db->select('*')->from('jenis_golongan')->where('id', $sub_golongan->jenis_golongan_id)->get()->row();
        $service = $this->db->select('*')->from('service')->where('project_id', $unit->project_id)->where('delete', 0)->where('active', 1)->get()->row();
        $range_lingkungan = $this->db->select('*')->from('range_lingkungan')->where('id', $sub_golongan->range_id)->get()->row();
        $range_lingkungan_detail = $this->db->select('*')->from('range_lingkungan_detail')->where('range_lingkungan_id', $range_lingkungan->id)->get()->result();

        $preprocessingData = (object)[
            'unit' => $unit,
            ...
            'range_lingkungan_detail' => $range_lingkungan_detail
        ];

        echo ("<pre>");
        print_r($preprocessingData);
        echo ("</pre>");
    }
}
