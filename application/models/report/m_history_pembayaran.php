...
class m_history_pembayaran extends CI_Model
{
    ...
    public function getRetribusi($kawasan,$blok,$periode_awal,$periode_akhir,$cara_bayar){
        ...
        $history_ipl = $this->db->select("
                                    ...
                                    isnull(t_pembayaran_detail.nilai_diskon,0) as nilai_diskon_ipl,
                                    kawasan.name as kawasan,
                                    blok.name as blok,
                                    unit.no_unit,
                                    m_pt.name as nama_pt,
                                ...
                                ->join("dbmaster.dbo.m_pt", "unit.pt_id = m_pt.pt_id", "LEFT")
    ...
    public function get_pembayaran_all($req)
    {
        $this->db->select("
            unit.pt_id,
            ISNULL(m_pt.name, '-') AS nama_pt,
            t_payment.cara_pembayaran_id,
            t_payment.cara_pembayaran_jenis_id,
            CASE ISNULL(bank.id, 0) 
                WHEN 0 THEN cara_pembayaran.name 
                ELSE CONCAT (cara_pembayaran.name, '-', bank.name) 
            END AS cara_bayar,
            ...
            service.code,
            FORMAT(t_payment.paid_date,'dd/MM/yyyy') AS tgl_bayar,
            FORMAT(t_payment.paid_date,'HH:mm:ss') AS jam_bayar,
            t_bill.periode_penggunaan,
            FORMAT(
                t_bill.periode_penggunaan,
                'MM/yyyy'
            ) AS periode_penggunaan_format,
            t_bill.periode_tagihan,
            FORMAT(
                t_bill.periode_tagihan,
                'MM/yyyy'
            ) AS periode_penagihan_format,
            t_bill_payment_map.nilai_dpp AS tagihan,
            (t_bill.nilai_cara_pembayaran + t_bill.nilai_cara_pembayaran_ppn) AS nilai_biaya_admin_cara_pembayaran,
            t_bill_payment_map.nilai_ppn,
            t_bill_payment_map.nilai_denda,
            t_bill_payment_map.diskon_total AS nilai_diskon,
            t_bill_payment_map.pemutihan_total AS nilai_tagihan_pemutihan,
            t_bill_payment_map.terbayar_total AS bayar,
            t_payment.no_kwitansi,
            t_payment.cetak AS count_print_kwitansi,
            RIGHT(CONCAT('00000000', unit.virtual_account), 8) AS virtual_account
        ");
        $this->db->from("t_payment");
        $this->db->join("t_bill_payment_map","t_bill_payment_map.t_payment_uuid = t_payment.uuid","INNER");
        $this->db->join("t_bill","t_bill.uuid = t_bill_payment_map.t_bill_uuid","INNER");

        $this->db->join("unit","t_payment.unit_id = unit.id","INNER");
        $this->db->join("blok","unit.blok_id = blok.id","INNER");
        $this->db->join("kawasan","blok.kawasan_id = kawasan.id","INNER");

        if (!cek_null_empty($req['jenis_service']))
        {
            $this->db->join("service","t_bill.service_id = service.id AND service.service_jenis_id = ".$req['jenis_service'],"INNER");
        }
        else
        {
            $this->db->join("service","t_bill.service_id = service.id","INNER");
        }

        $this->db->join("customer","customer.id = unit.pemilik_customer_id","INNER");

        $this->db->join("cara_pembayaran","t_payment.cara_pembayaran_id = cara_pembayaran.id","LEFT");
        $this->db->join("cara_pembayaran_jenis","t_payment.cara_pembayaran_jenis_id = cara_pembayaran_jenis.id","LEFT");
        $this->db->join("bank","cara_pembayaran.bank_id = bank.id","LEFT");
        $this->db->join("dbmaster.dbo.m_pt","unit.pt_id = m_pt.pt_id","LEFT");

        $this->db->join("t_payment_void","t_payment_void.t_payment_uuid = t_payment.uuid AND t_payment_void.deleted_at IS NULL","LEFT OUTER");

        if (!cek_null_empty($req['project_id']))
        {
            $this->db->where('unit.project_id', $req['project_id']);
        }
        if (!cek_null_empty($req['periode_awal']))
        {
            $this->db->where('t_payment.paid_date >=', $req['periode_awal']);
        }
        if (!cek_null_empty($req['periode_akhir']))
        {
            $this->db->where('t_payment.paid_date <=', $req['periode_akhir']);
        }
        if (!cek_null_empty($req['kawasan']))
        {
            $this->db->where('kawasan.id', $req['kawasan']);
        }
        if (!cek_null_empty($req['blok']))
        {
            $this->db->where('blok.id', $req['blok']);
        }
        if (!cek_null_empty($req['pt_id']))
        {
            $this->db->where('m_pt.pt_id', $req['pt_id']);
        }

        return $this->db->get();
    }
    
    public function get_pembayaran_lingkungan($req)
    {
        $this->db->select("
            unit.pt_id,
            ISNULL(m_pt.name, '-') AS nama_pt,
            ...
            CASE
                ...
                ELSE t_tagihan_lingkungan.periode
            ...
            t_pembayaran.nilai_biaya_admin_cara_pembayaran,
            CONVERT(INT,
                ROUND
                (
                    (
        ...
        $this->db->join("t_pembayaran","t_pembayaran_detail.t_pembayaran_id = t_pembayaran.id","INNER");
        $this->db->join("unit","t_pembayaran.unit_id = unit.id","INNER");
        $this->db->join("blok","unit.blok_id = blok.id","INNER");
        $this->db->join("kawasan","blok.kawasan_id = kawasan.id","INNER");
        $this->db->join("service","t_pembayaran_detail.service_id = service.id AND service.service_jenis_id = 1","INNER");
        $this->db->join("t_tagihan_lingkungan","t_pembayaran_detail.tagihan_service_id = t_tagihan_lingkungan.id","INNER");
        $this->db->join("t_tagihan_lingkungan_detail","t_tagihan_lingkungan.id = t_tagihan_lingkungan_detail.t_tagihan_lingkungan_id","INNER");
        $this->db->join("customer","customer.id = unit.pemilik_customer_id","INNER");

        $this->db->join("cara_pembayaran","t_pembayaran.cara_pembayaran_id = cara_pembayaran.id","LEFT");
        $this->db->join("cara_pembayaran_jenis","t_pembayaran.jenis_cara_pembayaran_id = cara_pembayaran_jenis.id","LEFT");
        $this->db->join("bank","cara_pembayaran.bank_id = bank.id","LEFT");
        $this->db->join("dbmaster.dbo.m_pt","unit.pt_id = m_pt.pt_id","LEFT");

        $this->db->where('t_pembayaran.is_void <>', '1');
        if (!cek_null_empty($req['project_id']))
        {
            $this->db->where('unit.project_id', $req['project_id']);
        }
        if (!cek_null_empty($req['periode_awal']))
        {
            $this->db->where('t_pembayaran.tgl_bayar >=', $req['periode_awal']);
        }
        if (!cek_null_empty($req['periode_akhir']))
        {
            $this->db->where('t_pembayaran.tgl_bayar <=', $req['periode_akhir']);
        }
        if (!cek_null_empty($req['kawasan']))
        {
            $this->db->where('kawasan.id', $req['kawasan']);
    ...
    public function get_pembayaran_air($req)
    {
        $this->db->select("
            unit.pt_id,
            ISNULL(m_pt.name, '-') AS nama_pt,
            ...
            CONVERT(
                ...
                (
                    ...
                    (
                        t_tagihan_air_detail.nilai_ppn / 100.0
                    ), 0
        ...
        $this->db->join("blok","unit.blok_id = blok.id","INNER");
        $this->db->join("kawasan","blok.kawasan_id = kawasan.id","INNER");
        $this->db->join("service","t_pembayaran_detail.service_id = service.id AND service.service_jenis_id = 2","INNER");
        $this->db->join("t_tagihan_air","t_pembayaran_detail.tagihan_service_id = t_tagihan_air.id","INNER");
        $this->db->join("t_tagihan_air_detail","t_tagihan_air.id = t_tagihan_air_detail.t_tagihan_air_id","INNER");
        $this->db->join("customer","customer.id = unit.pemilik_customer_id","INNER");
    ...
    public function get_pembayaran_lainnya($req)
    {
        $this->db->select("
            unit.pt_id,
            ISNULL(m_pt.name, '-') AS nama_pt,
            ...
            FORMAT(t_pembayaran.tgl_bayar,'HH:mm:ss') AS jam_bayar,
            CASE
                WHEN t_tagihan_layanan_lain_detail.periode_awal = t_tagihan_layanan_lain_detail.periode_akhir THEN t_tagihan_layanan_lain_detail.periode_awal 
        ...
        $this->db->join("blok","unit.blok_id = blok.id","INNER");
        $this->db->join("kawasan","blok.kawasan_id = kawasan.id","INNER");
        $this->db->join("service","t_pembayaran_detail.service_id = service.id","INNER");
        $this->db->join("t_tagihan_layanan_lain","t_pembayaran_detail.tagihan_service_id = t_tagihan_layanan_lain.id","INNER");
        $this->db->join("t_tagihan_layanan_lain_detail","t_tagihan_layanan_lain.id = t_tagihan_layanan_lain_detail.t_layanan_lain_tagihan_id","INNER");
        $this->db->join("customer","customer.id = unit.pemilik_customer_id","INNER");
