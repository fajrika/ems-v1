...
class m_core extends CI_Model {
    ...
    function menu(){
        ...
        else
        {
            $query = $this->db->select("m.*")
                            ->from("menu as m")
                            ->join("permission_menu pm", "pm.menu_id = m.id AND pm.level_id = ".$level_id, "INNER")
                            ->where("m.active", "1")
                            ->where("pm.read", "1")
                            ->order_by('m.id ASC')
                            ->get(); 
    ...
    function jabatan(){
        ...
        $row = $query->row(); 
        if (isset($row)) {
            $this->session->set_userdata('jabatan_name',$row->name);
        }
    ...
    function user(){
        $group_user_id = isset($this->session->userdata['group'])?$this->session->userdata['group']:'0';
        $query = $this->db->query("SELECT group_user.user_id, [user].name FROM group_user 
                                    INNER JOIN [user] ON [user].id = group_user.user_id 
                                    where group_user.id = $group_user_id");
        $row = $query->row();
        return isset($row)?$row:(object)[];
    }
