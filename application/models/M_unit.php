...
class m_unit extends CI_Model
{
    ...
    public function getSelect($id)
    {
        ...
        $query = $this->db->query("
            SELECT
                ...
                air.meter_id as air_pemeliharaan_air,
    ...
    public function get_sub_golongan($jenis_golongan_id)
    {
        ...
        $query = $this->db->query("
            SELECT 
                sub_golongan.*,
                case 
                    WHEN range_lingkungan.id is not null THEN concat(sub_golongan.name, ' - ',range_lingkungan.code) 
                    ELSE concat(sub_golongan.name, ' - ',range_air.code) 
                END as name
            FROM sub_golongan 
            join jenis_golongan
    ...
    public function getPT()
    {
        ...
        $result = $this->db
            ->select('m_pt.pt_id as id, m_pt.name')
            ->from('dbmaster.dbo.m_projectpt')
            ->join(
                'dbmaster.dbo.m_pt',
                'm_pt.pt_id = m_projectpt.pt_id'
            )
            ->where('m_projectpt.project_id', $project_id)
            ->distinct()->get()->result_array();
        return $result;
    }
    public function ajax_get_pt($project_id, $like)
    {
        $project = $this->db
            ->from("project")
            ->where("id", $project_id)
            ->get()->row();
        return $this->db
            ->select('m_pt.pt_id as id, m_pt.name as text')
            ->from("dbmaster.dbo.m_projectpt")
            ->join(
                'dbmaster.dbo.m_pt',
                'm_pt.pt_id = m_projectpt.pt_id'
            )
            ->where("m_projectpt.project_id", $project->source_id)
            ->like('name', $like)
            ->distinct()
            ->limit(10)
            ->get()->result();
    }
    public function ajax_get_kawasan($project_id, $like, $flag)
    {
        $data = $this->db
            ...
            ->get()->result();
        if ($flag == 0)
            array_unshift($data, (object)['id' => -1, 'text' => 'Tanpa Kawasan']);
        return $data;
    }
    public function ajax_get_blok($kawasan_id, $like)
    {
        return $this->db
            ->select('id,name as text')
            ->from('blok')
    ...
    public function ajax_get_customer($project_id, $like)
    {
        return $this->db
            ->select('id,name as text')
            ->from('customer')
    ...
    public function ajax_get_purpose_use($like)
    {
        return $this->db
            ->select('id,name as text')
            ->from('purpose_use')
    ...
    public function ajax_get_golongan($project_id, $like)
    {
        return $this->db
            ->select("id,concat(code, ' - ', description) as text")
            ->from('jenis_golongan')
            ->where('project_id', $project_id)
            ->like("concat(code, ' - ', description)", $like)
            ->limit(10)
            ->get()->result();
    }
    public function ajax_get_sub_golongan($golongan_id, $jenis, $like)
    {
        $data = $this->db
            ...
            ->from('sub_golongan')
            ->where('jenis_golongan_id', $golongan_id)
            ->where('range_flag', $jenis)
            ->limit(10);
        if ($jenis == 1)
            $data = $data->select("concat(sub_golongan.name, ' - ', range_lingkungan.code) as text")
                ->join('range_lingkungan', 'sub_golongan.range_id = range_lingkungan.id')
                ->like("concat(sub_golongan.name, ' - ', range_lingkungan.code)", $like);
        else
            $data = $data->select("concat(sub_golongan.name, ' - ', range_air.code) as text")
                ->join('range_air', 'sub_golongan.range_id = range_air.id')
                ->like("concat(sub_golongan.name, ' - ', range_air.code)", $like);
    ...
    public function getPemeliharaanMeterAir()
    {
        $project = $this->m_core->project();
        $query = $this->db->query("
            SELECT * FROM pemeliharaan_air where active =1 and project_id = $project->id   order by id asc
        ");
    ...
    public function get_log($id)
    {
        ...
        $query = $this->db->query("
            SELECT
                ...
                pemeliharaan_air.code as [Informasi Air -> Kode Sewa Meter],
            ...
            LEFT JOIN pemeliharaan_air
                ON pemeliharaan_air.id = unit_air.meter_id
