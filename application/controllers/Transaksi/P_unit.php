...
class p_unit extends CI_Controller
{
	...
	{
		$unit =
			$this->db->select("concat('1.',unit.id) as id, CONCAT(kawasan.name,'-',blok.name,'/',unit.no_unit,'-',customer.name,'-',unit.virtual_account) as text")
			->from('unit')
			->join(
				'blok',
				'blok.id = unit.blok_id'
			...
			->where('unit.project_id', $GLOBALS['project']->id)
			->where('unit.delete <>', 1)
			->where("CONCAT(kawasan.name,'-',blok.name,'/',unit.no_unit,'-',customer.name,'-',unit.virtual_account) like '%" . $this->input->get('data') . "%'")
			->limit(10)
			->get()->result();
		$data = (object)[];
		$data->result = [];
