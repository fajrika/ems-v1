...
class P_pembayaran_liason_officer extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->database();
        $this->load->model('transaksi_lain/m_pembayaran_liason_officer');
    ...
    {
        $data = $this->m_pembayaran_liason_officer->getAll();
        $this->load->view('core/header');
        $this->load->view('core/side_bar', ['menu' => $GLOBALS['menu']]);
        $this->load->view('core/top_bar', ['jabatan' => $GLOBALS['jabatan'], 'project' => $GLOBALS['project']]);
        $this->load->view('core/body_header', ['title' => 'Transaksi Service Lain > Liason Officer > Pembayaran', 'subTitle' => 'List']);
        $this->load->view('proyek/transaksi_lain/pembayaran_liason_officer/view' , ['data' => $data]);
    ...
    public function add()
    {
        ...
        $dataUnit = $this->m_registrasi_liason_officer->getUnit(); 
        $dataJenisService = $this->m_registrasi_liason_officer->getJenisService();     
        $this->load->view('core/header');
        $this->load->view('core/side_bar', ['menu' => $GLOBALS['menu']]);
        $this->load->view('core/top_bar');
        $this->load->view('core/body_header', ['title' => 'Transaksi Service Lain > Liason Officer > Registrasi', 'subTitle' => 'Add']);
        $this->load->view('proyek/transaksi_lain/registrasi_liason_officer/add', ['dataUnit' => $dataUnit, 'dataJenisService' => $dataJenisService ]);
        $this->load->view('core/body_footer');
        $this->load->view('core/footer');
    }
    ...
    {	
        ...
        {
        $status = $this->m_registrasi_liason_officer->save([
        ...
        $this->load->model('transaksi_lain/m_registrasi_liason_officer');
        $dataUnit = $this->m_registrasi_liason_officer->getUnit(); 
        $dataJenisService = $this->m_registrasi_liason_officer->getJenisService();   
        $this->load->view('core/header');
        $this->alert->css();
    ...
    public function lihat_unit()
    {
        ...
        echo json_encode($this->m_registrasi_liason_officer->getUnit2($pilih_unit));
    }
