...
class P_kirim_email  extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->database();
        $this->load->model('proyek/tools/m_kirim_email');
    ...
    {
        $dataJenisEmail = $this->m_kirim_email->getJenisEmail();
        $dataKawasan = $this->m_kirim_email->getKawasan();
        $this->load->view('core/header');
        $this->load->view('core/side_bar', ['menu' => $GLOBALS['menu']]);
        $this->load->view('core/top_bar', ['jabatan' => $GLOBALS['jabatan'], 'project' => $GLOBALS['project']]);
        $this->load->view('core/body_header', ['title' => 'Tools > Email > Kirim', 'subTitle' => 'List']);
        $this->load->view('proyek/tools/kirim_email/view' , ['dataJenisEmail' => $dataJenisEmail, 'dataKawasan' => 
        $dataKawasan]);
    ...
    public function lihat_template_email()
    {
         $this->load->model('proyek/tools/m_kirim_email');

        $type = $this->input->post('type');

        echo json_encode($this->m_kirim_email->getTemplateEmail($type));
    }
    ...
    public function lihat_blok()
    {
        ...
        echo json_encode($this->m_kirim_email->getBlok($kawasan_id));
    }
    ...
    public function lihat_unit()
    {
        ...
        $kawsan_id = $this->input->post('kawasan');


        $blok_id = $this->input->post('blok');


        echo json_encode($this->m_kirim_email->getUnit($kawasan_id, $blok_id));
    }
