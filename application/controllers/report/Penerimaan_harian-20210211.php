...
class Penerimaan_harian extends CI_Controller {
    ...
    public function get_data_summary_ajax()
    {
        ...
        $head = $this->sql_header($kawasan, $blok, $periode_awal, $periode_akhir, $pt_id);
        if ($head->num_rows() > 0) 
        {
            $query = $head->result();
            $join_unit_id = [];
    ...
    public function load_table()
    {
        ...
        if ($head->num_rows() > 0) 
        {
            ...
            foreach ($join_unit_id as $h) 
            {
                ...
                {
                    echo "<tr><td colspan='14' style='background-color: #f9f9f9; font-weight: 600;'>".$h->nama_pt."</td></tr>";
                }
                ...
                ?>
                <tr><td colspan="14" style="background-color: #c1ffe9; font-weight: 600;"><?=$h->cara_bayar;?></td></tr>
                <?php
                ...
                echo "
                <tr style='background-color: #ffffff;'>
                    <td colspan='13' align='right'>TOTAL BAYAR (Rp.)</td>
    ...
    public function sql_header($kawasan, $blok, $periode_awal, $periode_akhir, $pt_id)
    {
        $project_id = $GLOBALS['project']->id;
        if ($kawasan=='all') {
            $where_kawasan = "";
    ...
    public function sql_detail($kawasan, $blok, $periode_awal, $periode_akhir, $unit_id, $jenis_service, $cara_pembayaran_id, $cara_pembayaran_jenis_id, $pt_id)
    {
        $project_id = $GLOBALS['project']->id;
    ...
    public function export_excel()
    {
        ...
        $spreadsheet->getActiveSheet()->setCellValue('D2', "Service")->mergeCells("D2:D4")
                    ->getStyle('D2:D4')->applyFromArray($styleBorder);

        $spreadsheet->getActiveSheet()->setCellValue('E2', "Periode Penggunaan")->mergeCells("E2:E4")
                    ->getStyle('E2:E4')->applyFromArray($styleBorder);

        $spreadsheet->getActiveSheet()->setCellValue('F2', "Periode Penagihan")->mergeCells("F2:F4")
                    ->getStyle('F2:F4')->applyFromArray($styleBorder);

        $spreadsheet->getActiveSheet()->setCellValue('G2', "Nilai (Rp.)")->mergeCells("G2:N2")
                    ->getStyle('G2:N2')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('G3', "Pokok")->mergeCells("G3:G4")
                    ->getStyle('G3:G4')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('H3', "PPN")->mergeCells("H3:H4")
                    ->getStyle('H3:H4')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('I3', "Tagihan")
                    ->getStyle('I3')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('I4', "Pokok + PPN")
                    ->getStyle('I4')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('J3', "Denda")->mergeCells("J3:J4")
                    ->getStyle('J3:J4')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('K3', "Total")
                    ->getStyle('K3')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('K4', "Tagihan + Denda")
                    ->getStyle('K4')->applyFromArray($styleBorder)->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('L3', "Diskon")->mergeCells("L3:L4")
                    ->getStyle('L3:L4')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('M3', "Pemutihan")->mergeCells("M3:M4")
                    ->getStyle('M3:M4')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('N3', "Bayar")->mergeCells("N3:N4")
                    ->getStyle('N3:N4')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('O2', "No Kwitansi")->mergeCells("O2:O4")
                    ->getStyle('O2:O4')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('P2', "Cetak Ke-")->mergeCells("P2:P4")
                    ->getStyle('P2:P4')->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);

        $spreadsheet->getActiveSheet()->setCellValue('Q2', "Virtual Account")->mergeCells("Q2:Q4")
        ...
        if ($head->num_rows() > 0) 
        {
            ...
            foreach ($join_unit_id as $h) 
            {
                ...
                if (! in_array($nama_pt, $distinct_pt))
                {
                    $spreadsheet->getActiveSheet()->setCellValue('A'.$nomor, $h->nama_pt)->mergeCells("A".$nomor.":N".$nomor)->getStyle("A".$nomor.":N".$nomor)->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
                }
                array_push($distinct_pt, $nama_pt);
                ...
                $nomor++;
                $spreadsheet->getActiveSheet()->setCellValue('A'.$nomor, $h->cara_bayar)->mergeCells("A".$nomor.":N".$nomor)->getStyle("A".$nomor.":N".$nomor)->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
                ...
                foreach (array_reverse($detail->result()) as $d) 
                {
                    ...
                    if (!empty($tagihan))
                    {
                        ...
                        $spreadsheet->getActiveSheet()->setCellValue('D'.$nomor, $d->code)->getStyle('D'.$nomor)->applyFromArray($styleBorder);
                        $spreadsheet->getActiveSheet()->setCellValue('E'.$nomor, $periode_penggunaan)->getStyle('E'.$nomor)->applyFromArray($styleBorder);
                        $spreadsheet->getActiveSheet()->setCellValue('F'.$nomor, $periode_penagihan)->getStyle('F'.$nomor)->applyFromArray($styleBorder);
                        $spreadsheet->getActiveSheet()->setCellValue('G'.$nomor, $tagihan, 0)->getStyle('G'.$nomor)->applyFromArray($styleBorder);
                        ...
                        $spreadsheet->getActiveSheet()->setCellValue('J'.$nomor, ($d->code == 'Biaya Admin' ? '' : $denda))
                        ...
                        if(empty($nilai_diskon)){
                            $spreadsheet->getActiveSheet()->setCellValue('L'.$nomor, ($d->code == 'Biaya Admin' ? '' : $nilai_diskon))
                        ...
                        if(empty($nilai_tagihan_pemutihan)){
                            $spreadsheet->getActiveSheet()->setCellValue('M'.$nomor, ($d->code == 'Biaya Admin' ? '' : $nilai_tagihan_pemutihan))
                        ...
                        $spreadsheet->getActiveSheet()->setCellValue('N'.$nomor, $bayar)->getStyle('N'.$nomor)->applyFromArray($styleBorder)->getNumberFormat()->setFormatCode('#,##');
                        $spreadsheet->getActiveSheet()->setCellValue('O'.$nomor, $d->no_kwitansi)->getStyle('O'.$nomor)->applyFromArray($styleBorder)->getNumberFormat()->setFormatCode('#,##');
                        $spreadsheet->getActiveSheet()->setCellValue('P'.$nomor, $d->count_print_kwitansi)
                                    ->getStyle('P'.$nomor)->applyFromArray($styleBorder)->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
                        $spreadsheet->getActiveSheet()->setCellValue('Q'.$nomor, $d->virtual_account)
                ...
                $nomor++;
                $spreadsheet->getActiveSheet()->setCellValue('A'.$nomor, 'TOTAL BAYAR (Rp.)')->mergeCells("A".$nomor.":M".$nomor)->getStyle("A".$nomor.":M".$nomor)->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
                $spreadsheet->getActiveSheet()->setCellValue('N'.$nomor, $grand_total)->mergeCells("N".$nomor.":N".$nomor)->getStyle("N".$nomor.":N".$nomor)->applyFromArray($styleBorder)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
            }
            $nomor++;
        }
