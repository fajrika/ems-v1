...
class P_parameter_project extends CI_Controller
{
    ...
    public function ajax_save(){
        $project = $this->m_core->project();
        $entities = array('%21', '%2A', '%27', '%28', '%29', '%3B', '%3A', '%40', '%26', '%3D', '%2B', '%24', '%2C', '%2F', '%3F', '%25', '%23', '%5B', '%5D','%7B','%7D','+','%0D%0A','script');
        $replacements = array('!', '*', "'", "(", ")", ";", ":", "@", "&", "=", "+", "$", ",", "/", "?", "%", "#", "[", "]","{","}"," ",'\n','');
        $post_value = html_entity_decode(str_replace(array("script","span"), "", $this->input->post('value')));
        $value= urlencode($post_value);
        $value =  str_replace($entities, $replacements, $value);
        
        echo(json_encode($this->m_parameter_project->save($this->input->post(),$post_value,$this->input->get("id"),$project->id)));
    }
