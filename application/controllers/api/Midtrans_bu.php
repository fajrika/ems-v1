...
class Midtrans extends REST_Controller
{
    ...
    public function index_post($api_key = null)
    {
        ...
        $salesforce_id  = $this->post("order_id") ? $this->post("order_id") : null;
        ...
        $external_id_string = $checkout_salesforce->external_id ?? 0;
        ...
        $parse_status = [
            "Status__c" => "Pending", // Pending, Refund
            "Paid_Date__c" => $WaktuZulu // tidak di isi jika selain Paid
        ];

        if ($this->post("transaction_status") == 'settlement' || $this->post("transaction_status") == 'capture') {
        ...
        } elseif (in_array($this->post('transaction_status'), array('deny', 'cancel', 'expire'))) {
            $parse_status = [
                "Status__c" => "Expired"
            ];

            // insert data from sales_force.php
            ...
            $this->db->insert(
                'log_tp',
                [
                    'url' => 'https://ems.ciputragroup.com:11443/index.php/api/midtrans/midtrans_permission',
                    'type' => 1, //
                    'method' => 'POST',
                    'status_code' => '200',
                    'vendor_name' => 'MIDTRANS',
            ...
            $message = "$external_id_string - Success - Pending ";
        }
