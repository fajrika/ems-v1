...
class Erems extends REST_Controller
{
    ...
    public function kirim_post()
    {
        ...
        if ($val > 0) {
            ...
            if ($validasi_unit) {
                ...
                $this->db->update("unit", (array)$unit);
            } else {
                // validasi unit 2
                $validasi_unit = $this->db->select("id")
                    ->from("unit")
    ...
    public function have_bill_get($api_key){
        $uid = $this->get('uid');
        $url = "https://ems.ciputragroup.com:11443/index.php/api/erems/have_bill/$api_key/?uid=$uid";
        $request = json_encode($this->get());
        $path = 'Controller/API/EREMS/have_bill_get';

        $response = (object)[
            'status' => 200,
            'message' => 'Unit di temukan',
            'data' => (object)[
                'status_bill' => 0
            ]
        ];
        if($api_key == 'iukjblikBVLIKLBvliasfda'){
            $unit_id = $this->db->select("unit.id")
                ->from("unit")
                ...
                ->get()->row()->id ?? 0;
            if($unit_id > 0){
                $status_bill = $this->db->select('count(*) as c')->from("t_tagihan")->where('unit_id',$unit_id)->get()->row()->c ?? 0;
                $response->data->status_bill = $status_bill > 0 ? 1 : 0;
            }else{
                $response->status = 400;
                $response->message = 'Unit tidak ada di ems';
            }
            
        }
        $this->_store_log($url, 1, 'GET', $response->status, 'EREMS', $request, json_encode($response), $path);
