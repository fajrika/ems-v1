...
class P_pembayaran_tvi extends CI_Controller
{
    ...
    public function add()
    {
        ...
        $kode_pay = $project->code."/PEMBAYARANTVI/".date("Y")."/".$this->m_core->numberToRomanRepresentation($this->m_core->project()->id)."/".($this->m_pembayaran_tvi->last_id()+1);
    ...
    public function add_reg()
    {
       ...
       $id =  $this->input->get('id');
        ...
        $dataCaraPembayaran = $this->m_pembayaran_tvi->getCaraPembayaran();
        $dataTagihan = $this->m_pembayaran_tvi->getTagihan($id);
        $kode_pay = $project->code."/PEMBAYARANTVI/".date("Y")."/".$this->m_core->numberToRomanRepresentation($this->m_core->project()->id)."/".($this->m_pembayaran_tvi->last_id()+1);
       // var_dump($dataCaraPembayaran);
        $this->load->view('core/header');
        $this->load->view('core/side_bar', ['menu' => $GLOBALS['menu']]);
        ...
        $this->load->view('proyek/transaksi_lain/pembayaran_tvi/add_reg', [
            'dataRegistrasi' => $dataRegistrasi, 'dataCaraPembayaran' => $dataCaraPembayaran, 'kode_pay'=> $kode_pay,  'dataTagihan' => $dataTagihan
        ]);
        $this->load->view('core/body_footer');
        $this->load->view('core/footer');
    }
    ...
    public function pembayaran_biaya_tambahan()
    {
        ...
        $dataRegistrasi = $this->m_pembayaran_tvi->getRegistrasi2($id);
        $dataCaraPembayaran = $this->m_pembayaran_tvi->getCaraPembayaran();
        $dataBiayaTambahan = $this->m_pembayaran_tvi->getBiayaTambahan($id);
        $kode_pay = $project->code."/PEMBAYARANTVI/".date("Y")."/".$this->m_core->numberToRomanRepresentation($this->m_core->project()->id)."/".($this->m_pembayaran_tvi->last_id()+1);
       // var_dump($dataCaraPembayaran);
       ...
       $this->load->model('m_log');
        ...
        $this->load->view('proyek/transaksi_lain/pembayaran_tvi/add_biaya', [
            ...
            'dataBiayaTambahanDetail' => $dataBiayaTambahanDetail, 'data' => $data,  'dataBiayaTambahan' => $dataBiayaTambahan
        ]);
        $this->load->view('core/body_footer');
        $this->load->view('core/footer');
    }
    ...
    public function save_biaya_tambahan()
    {
        ...
        $dataBiayaTambahanDetail = $this->m_biaya_tambahan_tvi->get_biaya_tambahan_detail($this->input->get('id'));
        $dataCaraPembayaran = $this->m_pembayaran_tvi->getCaraPembayaran();
        $kode_pay = $project->code."/PEMBAYARANTVI/".date("Y")."/".$this->m_core->numberToRomanRepresentation($this->m_core->project()->id)."/".($this->m_pembayaran_tvi->last_id());
       // var_dump($dataCaraPembayaran);
    ...
    public function lihat_tagihan()
    {
        ...
        echo json_encode($this->m_pembayaran_tvi->getTagihan($registrasi_id));
    }
