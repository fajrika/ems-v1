...
<div class="x_content">
	<br>
	<form id="form" data-parsley-validate="" enctype="multipart/form-data" class="form-horizontal form-label-left" novalidate="" method="post" action="<?=site_url()?>/transaksi_lain/p_registrasi_liaison_officer/aksi_edit?id=<?=$this->input->get('id');?>">
		...
		<div class="col-md-6">
			...
			<div class="form-group two">
				...
				<div class="col-md-9 col-sm-9 col-xs-12">
					<select name="peruntukan_loi_id" id="peruntukan" required="" disabled class="peruntukan form-control select2">
		...
		<div id="view_data">
			...
			<div class="col-md-6 col-sm-6 col-xs-12 paket_internet">
				<div class="form-group two">
					...
					<div id="lihat_paket">
						<div class="col-md-9 col-sm-9 col-xs-12">
							<input type="text" name="kode" id="kode" readonly class="kode form-control" value="<?=$dataSelect->kode_paket?>"/>
						</div>
					</div>
				</div>
				<div class="form-group two">
					...
					<div class="col-md-9 col-sm-9 col-xs-12">
						<input type="text" name="nilai_registrasi" value="<?=number_format($dataSelect->nilai_registrasi)?>" id="nilai_registrasi" readonly class="form-control ">
					</div>
				</div>
                <div class="form-group two">
					<label class="control-label col-md-3 col-sm-3 col-xs-12">Harga Pengurusan Jenis (Rp)</label>
					<div class="col-md-9 col-sm-9 col-xs-12">
						<input type="text" name="nilai_jasa" value="<?=number_format($dataSelect->nilai_jasa)?>" id="nilai_jasa" readonly class="form-control currency">
					</div>
				</div>
			</div>
			<div class="col-md-6 col-sm-6 col-xs-12 paket_internet">
				<div class="form-group two">
					...
					<div class="col-md-9 col-sm-9 col-xs-12">
						<input type="text" name="nilai_prakiraan" readonly="" id="nilai_prakiraan" value="<?=number_format($dataSelect->nilai_prakiraan)?>" class="form-control currency">
					</div>
				</div>
				...
				<div class="form-group two">
					...
					<div class="col-md-9 col-sm-9 col-xs-12">
						<input type="text" name="" readonly="" id="subtotal" name="subtotal" value="<?=number_format($dataSelect->nilai_registrasi+$dataSelect->nilai_jasa+$dataSelect->nilai_prakiraan)?>" class="form-control currency">
					</div>
				</div>
			</div>
			<div class="clearfix"></div>
			<h4 id="label_transaksi" hidden>Item Charge</h4>
			<hr id="hr_item">
					...
					<div class="form-group two ">
						<table class="table table-responsive">
							...
							<tbody id="tbody">
								<tr class="isi">
									<td>
										<input id="nama_item" name="nama_item[]" required="" value="" class="nama_item form-control col-md-1 col-xs-12">
									</td>
									<td>
										<input id="volume" name="volume[]" required="" value="" class="volume form-control col-md-1 col-xs-12 currency">
									</td>
									<td>
										<input id="satuan" name="satuan[]" required="" class="satuan form-control col-md-1 col-xs-12">
									</td>
									<td>
										<input type="file" name="bukti_survei[]" class="bukti_survei form-control col-md-1 col-xs-12">
									</td>
									<td>
										<a class='delete btn btn-danger' id="btn-delete-paket"><i class='fa fa-trash'></i> </a>
									</td>
								</tr>
							</tbody>
						</table>
			...
			<div class="row">
				<div class="col-md-12">
					...
					<div class="col-md-6">
						<div class="form-group two">
							...
							<div class="col-md-9 col-sm-9 col-xs-12">
								<div class='input-group date '>
									...
									id="expired_date" placeholder="Masukkan Tanggal Expired" value="<?=$dataSelect->expired_date?>"> <span class="input-group-addon">
										<span class="glyphicon glyphicon-calendar"></span>
									</span>
								</div>
							</div>
						...
						<div class="form-group two">
							<label class="control-label col-md-3 col-sm-3 col-xs-12">Harga Paket Lama (Rp)</label>
							<div class="col-md-9 col-sm-9 col-xs-12">
								<input type="text" name="" readonly="" id="harga_paket" name="harga_paket" value="<?=number_format($dataSelect->harga_paket)?>" class="form-control currency">
							</div>
						</div>
								<div class="form-group two">
							<label class="control-label col-md-3 col-sm-3 col-xs-12">Total Paket LOI</label>
							<div class="col-md-9 col-sm-9 col-xs-12">
								<input type="text" name="totalpaket" readonly="" value="<?=number_format($dataSelect->nilai_registrasi+$dataSelect->nilai_jasa+$dataSelect->nilai_prakiraan)?>" id="totalpaket" class="form-control currency">
							</div>
						</div>
						<div class="form-group two">
						...
						<div class="form-group two">
							...
							<div class="col-md-9 col-sm-9 col-xs-12">
								<input type="text" name="total_bayar" readonly="" id="total_bayar" value="<?=number_format($dataSelect->nilai_registrasi+$dataSelect->nilai_jasa+$dataSelect->nilai_prakiraan+$dataSelect->deposit_masuk)?>" class="form-control currency">
							</div>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	$("#jenis").change(function(){
		...
		$.ajax({
			...
			success: function (data) {
				...
				$.each(data, function (key, val) {
					$("#paket").append("<option value='" + val.id + "|" + val.kode + "|" + val.nilai_registrasi + "|" + val.nilai_jasa +"|" + val.nilai_prakiraan + "'>" + val.nama + "</option>");
				});
			}
		});
	});
	...
	$("#paket").change(function(){
		...
		var total = parseInt(nilai_registrasi) + parseInt(nilai_jasa) + parseInt(nilai_prakiraan);
