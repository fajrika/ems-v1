...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?=site_url()?>/transaksi_lain/P_st_liaison_officer'">
...
<div class="x_content">
	<br>
	<form id="form" data-parsley-validate="" enctype="multipart/form-data" class="form-horizontal form-label-left" novalidate="" method="post" action="<?=site_url()?>/transaksi_lain/p_st_liaison_officer/save?id=<?=$this->input->get('id');?>">
		...
		<div id="view_data">
			...
			<div class="col-md-6">
				...
				<div class="form-group two">
					<label class="control-label col-md-3 col-sm-3 col-xs-12">Total Pemakaian Deposit</label>
					<div class="col-md-9 col-sm-9 col-xs-12">
						<input type="text" class="form-control currency" readonly id="deposit_pakai" name="deposit_pemakaian" value='<?=number_format($dataSelect->deposit_pemakaian + $dataSelect->deposit_pemakaian2)?>'>
					</div>
				</div>
				<?php
					$total = ($dataSelect->deposit_masuk-$dataSelect->deposit_pemakaian) -$dataSelect->deposit_pemakaian2;
					if ($total<0) {
						?>

				<div class="form-group two">
					<label class="control-label col-md-3 col-sm-3 col-xs-12">Kekurangan Deposit</label>
					<div class="col-md-6 col-sm-6 col-xs-12">
						<input type="text" class="form-control currency" name="kurang" id="kurang" readonly value="<?=number_format($total*-1)?>">
					</div>
					<label class="control-label col-md-3 col-sm-3 col-xs-12">Sudah Bayar</label>
				</div>
				...
				<div class="form-group two">
					...
					<div class="col-md-9 col-sm-9 col-xs-12">
						<input type="text" class="form-control currency" name="sisa_deposit" id="sisa_deposit" readonly value="<?=number_format(($total<0)?0:$total)?>">
					</div>
				</div>
						...
						<table class="table table-responsive">
							<thead>
								<tr>
									...
									<!-- <th class="col-md-3">Total Satuan</th> -->
								</tr>
							</thead>
							<tbody>
								<tr>
									...
									<td></td>
									<td></td>
									<td hidden><center>TOTAL</center></td>
									<td hidden><input name="hrg_prediksi" id="hrg_prediksi" disabled required="" class="form-control col-md-1 col-xs-12 currency"></td>
								</tr>
								...
								<tr class="isi">
									...
									<td>
										<input id="harga_satuan" name="harga_satuan[]" value="<?=number_format($v['harga_satuan'])?>" disabled class="harga_satuan form-control col-md-1 col-xs-12 currency">
									</td>
									<!-- <td>
										<input id="total" name="total[]" disabled class="total form-control col-md-1 col-xs-12 currency">
									</td> -->
									<td hidden>
										<input name="id_item[]" value="<?=$v['id']?>"/>
									</td>
								</tr>
								<?php }?>
							</tbody>
			...
			<div class="clearfix"></div>
			<h4 id="label_transaksi">Item Charge Tambahan</h4>
			<hr id="hr_item">
			<div class="row">
				<div class="col-md-12">
					<div class="form-group two ">
						<table class="table table-responsive">
							...
							<tbody>
								...
								<?php foreach($dataItemChargeTambah as $v){?>
								<tr class="isi">
									<td>
										<input id="nama_item" name="nama_item[]" disabled value="<?=$v['nama_item']?>" class="nama_item form-control col-md-1 col-xs-12">
									</td>
			...
			<div class="row">
				...
				<div class="col-md-12">
					<div class="col-md-6">
                        <div class="form-group two start ">
							...
							<div class="col-md-9 col-sm-9 col-xs-12">
								<div class='input-group date '>
									...
									id="tanggal_pemasangan_mulai" value="<?= $dataSelect->tanggal_rencana_survei?>" disabled placeholder="Masukkan Tanggal Pasang Instalasi"> <span class="input-group-addon">
										<span class="glyphicon glyphicon-calendar"></span>
									</span>
								</div>
							</div>
						...
						<div class="form-group two start ">
							<label class="control-label col-md-3 col-sm-3 col-xs-12">Tanggal Rencana Serah Terima</label>
							<div class="col-md-9 col-sm-9 col-xs-12">
								<div class='input-group date '>
					...
					<div class="col-md-6">
						<div class="form-group two">
							<label class="control-label col-md-3 col-sm-3 col-xs-12">Sisa Deposit</label>
							<div class="col-md-9 col-sm-9 col-xs-12">
								<input type="text" class="form-control currency" name="sisa_deposit" 
								value="<?=number_format(($total<0)?0:$total)?>" id="sisa_deposit" disabled>
							</div>
						</div>
                        <div class="form-group two ">
                        ...
                        <div class="form-group two ">
                            ...
                            <div class="col-md-4 col-sm-4 col-xs-12">
                                <div class='input-group date '>
                                    <input type="text" class="form-control" value='<?= $dataSelect->tanggal_instalasi?>' disabled placeholder="Masukkan Tanggal Document"> <span class="input-group-addon">
                                        <span class="glyphicon glyphicon-calendar"></span>										
                                </div>
                            </div>
                            <div class="col-md-4 col-sm-4 col-xs-12">
						...
						<div class="form-group two start ">
							<label class="control-label col-md-3 col-sm-3 col-xs-12">Tanggal Realisasi Serah Terima</label>
							<div class="col-md-4 col-sm-4 col-xs-12">
								<div class='input-group date '>
									<input type="text" class="form-control datetimepicker" name="tanggal_aktifasi" value="<?= date('d-m-Y') ?>"> <span class="input-group-addon">
										<span class="glyphicon glyphicon-calendar"></span>
									</span>
								</div>
                            </div>
						...
						<div class="form-group two start ">
							<label class="control-label col-md-3 col-sm-3 col-xs-12">Action Serah terima</label>
							<div class="col-md-4 col-sm-4 col-xs-12">
								<select name="action_st" id="kategori" class="form-control select2">
									
									<?php if ($total>0) {?>
									<option value="2">withdrawal</option>	
									<option value="3">Mutasi Deposit</option>	
									<?php }else{ ?>
									<option value="1">Serah Terima</option>	
									<?php } ?>
								</select>
                            </div>
						</div>
...
<script type="text/javascript">
	...
	$("body").on("keyup",".harga_satuan",function(){
		var id = $(this).attr("data-id");
		var vol = "#volume-"+id;
		var volume = $(vol).val();
		var hrg = "#harga_satuan-"+id;
		var hrg_satuan = $(hrg).val();
		var tot = "#total-"+id;
		var total = $(tot).val();
		// var tes = harga_satuan+id;
		// alert(hrg_satuan);
		$(this).val(formatNumber($(this).val()));
		$(this).parent().parent().children().eq(4).children().val(
			formatNumber(
				unformatNumber($(this).val()) * 
				unformatNumber($(this).parent().parent().children().eq(1).children().val())
			)
		);
		var val = 0;
		for(i = 0 ; i < $(".isi").length ; i++){
			val += parseInt(unformatNumber($(".isi").eq(i).children().eq(4).children().val()));
		}
		$("#hrg_prediksi").val(formatNumber(val));
		var hrg_prediksi = $("#hrg_prediksi").val();
		$("#deposit_pakai").val(hrg_prediksi);
