		...
		$("#btn-load-blok").click(function() {
			...
			if ($("#kawasan").val() != null) {
				$.ajax({
					...
					url: "<?= site_url() ?>/Transaksi/P_belum_tertagih/ajax_get_blok2",
					dataType: "json",
					...
					for (var i = 0; i < data.length; i++) {
							...
							$("#tbody_unit").append(
								...
								"<td class='col-md-1'><a href='<?= site_url(); ?>/Transaksi/P_belum_tertagih/unit?id=" + data[i].blok_id + "&periode="+$("#periode").val()+"' class='save-row btn btn-primary' unit_id='" + data[i].id + "'><i class='fa fa-book'></i></a></td>" +
								"</tr>");
