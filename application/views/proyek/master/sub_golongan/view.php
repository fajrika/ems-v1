...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?=site_url(); ?>/P_master_sub_golongan/add'">
					...
					<tbody>
                        <?php 
                            ...
                            foreach ($data as $key => $v) {
                                ...
                                echo '<tr>';
                                echo "<td>$i</td>";
                                echo "<td>$v[gol_code]</td>";
                                echo "<td>$v[code]</td>";
                                echo "<td>$v[name]</td>";
                                echo "<td>$v[jenis_service]</td>";
                                echo "<td>$v[range_code]</td>";
                                echo '<td>'.number_format($v['minimum_pemakaian']).'</td>';
                                echo '<td>'.number_format($v['administrasi']).'</td>';
                                echo '<td>';
                                echo $v['active'] ? 'Aktif' : 'Tidak Aktif';
                                echo '</td>';
                                echo "
									<td>
									<a href='".site_url()."/P_master_sub_golongan/edit?id=$v[id]' class='btn btn-primary col-md-10'>
...
<script>
	function confirm_modal(id) {
		...
		document.getElementById('delete_link_m_n').setAttribute("href",
			"<?= site_url('P_master_sub_golongan/delete?id=" + id + "'); ?>");
