...
<div class="x_content">
	...
	<form id="form-cara-pembayaran" autocomplete="off" class="form-horizontal form-label-left" method="post">
        ...
        <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
            	<input type="hidden" name="jenis_cara_pembayaran_id" value="<?=abs($dataSelected->jenis_cara_pembayaran_id);?>">
                <select type="text" id="jenis_cara_pembayaran" required="required" class="select2 form-control col-md-7 col-xs-12" name="jenis_cara_pembayaran" style="width:100%" placeholder="--Pilih Jenis Cara Pembayaran--" disabled require>
					...
					<?php foreach ($dataJenisCaraPembayaran as $v):?>
						<?php if($v->id == abs($dataSelected->jenis_cara_pembayaran_id)):?>
        ...
        <div class="form-group">
            <label class="control-label col-md-3 col-sm-12 col-xs-12">Status</label>
            <div class="col-md-6 col-sm-6 col-xs-12">
                <div class="checkbox">
                    <label id="label_status">
                        <p style="display:contents">Disable</p>
                            <input id="status" type="checkbox" class="js-switch disabled-form" name="status_flag" value="1" disabled 
                            	<?php
                            		if ($dataSelected->jenis_cara_pembayaran_id>-1)
                            		{
                            			echo "checked";
                            		}
                            	?>
                             />
                        <p style="display:contents">Enable</p>
                    </label>
                </div>
            </div>
        </div>
...
<div class="x_panel">
	...
	<div class="x_content">
		<br>
		<table class="table table-striped jambo_table tableDT2">
			<thead>
				<tr>
					<th>No</th>
					<th>Waktu</th>
...
<script type="text/javascript">
	...
	$(function () {
		...
		$('#form-cara-pembayaran').on('submit', function (e) {
			...
			data.push({
				name:"status_flag", value:($('#form-cara-pembayaran input[name="status_flag"]').is(':checked') ? 1 : 0)
			});
