...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?= site_url(); ?>/P_master_paket_loi/add'">
		...
		<tbody>
			<?php foreach ($data as $key => $v) : ?>
				<tr>
					...
					<td><?= number_format($v->nilai_admin) ?></td>
					<td><?= number_format($v->uang_jaminan) ?></td>
					<td><?= $v->follow_up ?></td>
					<td><?= number_format($v->jumlah_outflow) ?></td>
					<td class='col-md-1'>
						<a href='<?= site_url("P_master_paket_loi/edit?id=$v->id") ?>' class='btn btn-md btn-primary col-md-12'>
	...
	$(function() {
		...
		$("#btn-delete").click(function() {
			$.ajax({
				...
				url: "<?= site_url('P_master_paket_loi/ajax_delete') ?>",
				dataType: "json",
