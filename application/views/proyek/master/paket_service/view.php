...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?=site_url(); ?>/P_master_paket_service/add'">
			...
			<div class="card-box table-responsive">
				...
				<tfoot id="tfoot" style="display: table-header-group">
						...
						<th>No</th>
							...
							<th>Biaya Satuan Restrasi</th>
							<th>Biaya Satuan Tanpa Restrasi</th>
							<th>Status</th>
							<th hidden>Action</th>
							<th hidden>Delete</th>
						</tr>
               ...
               $i = 0;
                foreach ($data as $key => $v) {
                    ...
                    echo "<td>$v->code</td>";
                    echo "<td>$v->name</td>";
                    echo '<td>'.number_format($v->biaya_satuan_langganan).'</td>';
                    echo '<td>'.number_format($v->biaya_satuan_tanpa_langganan).'</td>';
                    echo '<td>';
                    echo $v->active ? 'Aktif' : 'Tidak Aktif';
                    echo '</td>';
                    echo "
                        <td class='col-md-1'>
                          <a href='".site_url()."/P_master_paket_service/edit?id=$v->id' class='btn btn-sm btn-primary col-md-12'>
...
<script>
	function confirm_modal(id) {
		...
		document.getElementById('delete_link_m_n').setAttribute("href",
			"<?= site_url('P_master_paket_service/delete?id=" + id + "'); ?>");
