...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?=site_url(); ?>/P_master_pemeliharaan_meter_listrik/add'">
			...
			<div class="card-box table-responsive">
             ...
             $i = 0;
                foreach ($data as $key => $v) {
                    ...
                    echo"<td>$i</td>";
                    echo"<td>$v[code]</td>";
                    echo'<td>'.number_format($v['daya']).'</td>';
                    echo'<td>'.number_format($v['harga']).'</td>';
                    echo'<td>'.number_format($v['biaya_pasang_baru']).'</td>';
                    ...
                    echo"
                        <td class='col-md-1'>
                          <a href='".site_url()."/P_master_pemeliharaan_meter_listrik/edit?id=$v[id]' class='btn btn-sm btn-primary col-md-12'>
...
<script>
	function confirm_modal(id) {
		...
		document.getElementById('delete_link_m_n').setAttribute("href",
			"<?= site_url('P_master_pemeliharaan_meter_listrik/delete?id=" + id + "'); ?>");
