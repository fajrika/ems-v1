...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?= site_url(); ?>/P_master_kategori_loi/add'">
		...
		<tbody>
			<?php foreach ($data as $k => $v) : ?>
				<tr>
					...
					<td><?= $v->kode ?></td>
					<td><?= $v->nama ?></td>
					<?php if ($v->project_id != 0) : ?>
						<td class='col-md-1'>
							<a href='<?=site_url("P_master_kategori_loi/edit?id=$v->id")?>' class='btn btn-md btn-primary col-md-12'>
	...
	<script>
		function confirm_modal(id) {
			...
			document.getElementById('delete_link_m_n').setAttribute("href",
				"<?= site_url('P_master_kategori_loi/delete?id=" + id + "'); ?>");
