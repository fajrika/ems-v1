...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?=site_url(); ?>/P_master_item_tvi/add'">
	...
	<table id="tableDT2" class="table table-striped jambo_table bulk_action">
		...
		<tbody>
            ...
            foreach ($data as $key => $v) {
				...
				if($v->status_item=='1'){
					echo "<td>Dipinjamkan</td>";
				}else{
					echo "<td>Hak Milik</td>";
				}
                echo "
                        <td class='col-md-1'>
                        <a href='".site_url()."/P_master_item_tvi/edit?id=$v->id' class='btn btn-md btn-primary col-md-12'>
	...
	<script>
		function confirm_modal(id) {
			...
			document.getElementById('delete_link_m_n').setAttribute("href",
				"<?= site_url('P_master_item_tvi/delete?id=" + id + "'); ?>");
