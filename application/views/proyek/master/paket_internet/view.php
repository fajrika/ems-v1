...
<div style="float:right">
    <h2>
        <button class="btn btn-primary" onClick="window.location.href='<?= site_url(); ?>/P_master_paket_internet/add'">
				...
				<th hidden>Delete</th>
                    ...
                    <tbody>
						...
						foreach ($data as $key => $v) {
							...
							echo "<td>$v[code]</td>";
							echo "<td>$v[name]</td>";
							echo '<td>' . number_format($v['bandwidth']) . '</td>';
							echo '<td>' . number_format($v['harga_jual']) . '</td>';
							...
							echo "
                        <td class='col-md-1'>
                          <a href='" . site_url() . "/P_master_paket_internet/edit?id=$v[id]' class='btn btn-sm btn-primary col-md-12'>
...
<script>
    function confirm_modal(id) {
        ...
        document.getElementById('delete_link_m_n').setAttribute("href", "<?= site_url('P_master_paket_internet/delete?id=" + id +
			"'); ?>");
