...
<div style="float:right">
    <h2>
        ...
        <button class="btn btn-success" onclick="location.href='<?= site_url(); ?>/P_master_metode_penagihan/edit?id=<?= $this->input->get('id') ?>';">
...
<div class="x_content">
    <br>
    <form id="form" data-parsley-validate="" class="form-horizontal form-label-left" novalidate="" method="post" action="<?= site_url(); ?>/P_master_metode_penagihan/edit?id=<?= $this->input->get('id'); ?>">
        <div id="div_jenis_metode_penagihan" class="form-group">
            ...
            <div class="col-md-6 col-sm-12 col-xs-9">
                <select type="text" id="jenis_metode_penagihan" required="required" class="select2 form-control col-md-7 col-xs-12" name="jenis_metode_penagihan" style="width:100%" placeholder="--Pilih Jenis Meotde Penagihan--" disabled>
                    <option disabled selected>-- Pilih Jenis Metode Penagihan --</option>
                    <?php foreach ($dataMetodePenagihanJenis as $v) : ?>x
                        <?php if ($v->id == $data_select->metode_penagihan_jenis_id) : ?>
        ...
        <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
                <input type="text" id="metode_penagihan" name="metode_penagihan" required="required" class="form-control col-md-7 col-xs-12" value=<?= $data_select->name; ?> disabled>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
                <input type="text" id="biaya_admin" name="biaya_admin" required="required" class="form-control col-md-7 col-xs-12 currency" value=<?= $data_select->biaya_admin; ?> disabled>
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
                <select type="text" id="coa" required="required" class="select2 form-control col-md-7 col-xs-12" name="coa" style="width:100%" placeholder="--Pilih PT - COA Service--" disabled>
                    <option value="" selected="" disabled="">--Pilih PT COA--</option>
                    <?php
                        foreach ($dataPTCOA as $key => $c) {
                            if ($data_select->coa_mapping_id == $c->id) {
                                echo "<option value='$c->id' selected>$c->ptName $c->coaCode - $c->coaName </option>";
                            } else {
                                echo "<option value='$c->id'>$c->ptName $c->coaCode - $c->coaName </option>";
                            }
                        }
                    ?>
...
<script type="text/javascript">
    $(function() {
        ...
        $("#btn-update").click(function() {
            ...
            $("#metode_penagihan").removeAttr("disabled");
            $("#biaya_admin").removeAttr("disabled");
            $("#coa").removeAttr("disabled");
            $("#keterangan").removeAttr("disabled");
            $("#status").removeAttr("disabled");
            $("#jenis_metode_penagihan").removeAttr("disabled");
