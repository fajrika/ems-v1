...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?=site_url(); ?>/P_master_metode_penagihan/add'">
		...
		<button class="btn btn-success" onClick="window.location.href='<?=site_url(); ?>/P_master_metode_penagihan'">
					...
					<tbody>
						<?php
							...
							foreach ($data as $key => $v) {
								...
								echo "<td>$i</td>";
								echo"<td>$v[code]</td>";
								echo "<td>$v[coa_code]</td>";
								echo "<td>$v[coa_name]</td>";
								echo "<td>$v[pt_name]</td>";
								echo "<td>$v[name]</td>";
								...
								echo"
									<td class='col-md-1'>
									<a href='".site_url()."/P_master_metode_penagihan/edit?id=$v[id]' class='btn btn-sm btn-primary col-md-12'>
...
<script>
	function confirm_modal(id) {
		...
		document.getElementById('delete_link_m_n').setAttribute("href", "<?= site_url('P_master_metode_penagihan/delete?id=" +
			id + "'); ?>");
