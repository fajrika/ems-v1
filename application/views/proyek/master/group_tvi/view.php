...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?=site_url(); ?>/P_master_group_tvi/add'">
			   ...
			   <th>Kode</th>
                ...
                foreach ($data as $key => $v) {
					...
					if ($v['project_id']===$project->id) {
						echo"
							<td class='col-md-1'>
							<a href='".site_url()."/P_master_group_tvi/edit?id=$v[id]' class='btn btn-sm btn-primary col-md-12'>
...
<script>
	function confirm_modal(id) {
		...
		document.getElementById('delete_link_m_n').setAttribute("href", "<?= site_url('P_master_group_tvi/delete?id=" + id +
			"'); ?>");
