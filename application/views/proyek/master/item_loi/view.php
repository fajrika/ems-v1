...
<div style="float:right">
	<h2>
		<button class="btn btn-primary" onClick="window.location.href='<?= site_url(); ?>/P_master_item_loi/add'">
	...
	<table id="tableDT2" class="table table-striped jambo_table bulk_action">
		...
		<tbody>
			<?php foreach ($data as $key => $v) : ?>
				<tr>
					<td><?= $v->name ?></td>
					<td><?= number_format($v->jumlah_satuan) ?></td>
					<td class='col-md-1'>
						<a href='<?= site_url("P_master_item_loi/edit?id=$v->id") ?>' class='btn btn-md btn-primary col-md-12'>
	...
	$(function() {
		...
		$("#btn-delete").click(function() {
			$.ajax({
				...
				url: "<?= site_url('P_master_item_loi/ajax_delete') ?>",
				dataType: "json",
