    ...
    <script>
            ...
            var table = $('.tableDT').DataTable();
                if (<?= permission() ? permission()->create == 0 ? 1 : 0 : 0 ?>) {
                    $.each($("button"), function(k, v) {
                        text_object = $(this).html().toLowerCase();
                        if (text_object.toLowerCase().includes("tambah")||text_object.includes("kirim"))
                            $(this).remove();
                    });
                    $.each($("a"), function(k, v) {
                        text_object = $(this).html().toLowerCase();
                        if (text_object.includes("submit")||text_object.includes("buat pemutihan"))
                            $(this).remove();
                    });
                }
                if (<?= permission() ? permission()->delete == 0 ? 1 : 0 : 0 ?>) {
                    ...
                    table.columns().every(function() {
                        if (this.header().textContent.toLowerCase().includes('delete'))
                ...
                if (<?= permission() ? permission()->update == 0 ? 1 : 0 : 0 ?>) {
                    // var index = 0;
                    // table.columns().every(function() {
                    //     if (this.header().textContent.toLowerCase().includes('action'))
                    //         table.column(index).visible(false);
                    //     index++;
                    // })
                    // console.log(index);
                    ...
                    $.each($("button"), function(k, v) {
                        text_object = $(this).html().toLowerCase();
                        if (text_object.includes("edit"))
                            $(this).remove();
                    })
                    $.each($('input'), function(k, v) {
                        if ($(this).val().toLowerCase().includes('edit'))
                            $(this).remove();
                    })
                }
            var table2 = $('.bulk_action').DataTable();
                if (<?= permission() ? permission()->delete == 0 ? 1 : 0 : 0 ?>) {
                    ...
                    table2.columns().every(function() {
                        if (this.header().textContent.toLowerCase().includes('delete'))
                            table2.column(index).visible(false);
                        index++;
                    })
                    console.log(index);
                }
        ...
        var jabatan, project;
        $("#saveJP").click(function() {
            url = '<?= site_url() ?>/core/changeJP_ajax';
            $.ajax({
                type: "post",
                url: url,
                data: {jabatan:$('#form_change_jp select[name="jabatan"]').val(), project:$('#form_change_jp select[name="project"]').val()},
                dataType: "json",
                success: function(data) {
                    ...
                    else
                    {
                        new PNotify({
                            title: 'Gagal',
                            text: 'Data Jabatan Project gagal dirubah',
                            type: 'danger',
                            styling: 'bootstrap3'
                        });
                    }
        ...
        $("#changeJP").click(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $("#saveJP").attr('style','width: 75px');
                    $("#cancel_changeJP").attr('style','width: 75px');
        ...
        $("#cancel_changeJP").click(function() {
            $('#jabatan').html(jabatan);
            $('#project').html(project);
            $('#jabatan').attr('disabled','disabled');
            $('#project').attr('disabled','disabled');
            
            $("#saveJP").attr("style", "display:none");
            $("#cancel_changeJP").attr("style", "display:none");
            $("#changeJP").attr('style','width: 75px');
        });
